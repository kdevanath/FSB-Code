...
namespace Yammer {
  ...
  extern "C" void __signalHandler(int sig)
  {
    ...
    if (logFile) {
      ACE_HANDLE fd = ACE_OS::open(logFile, O_WRONLY | O_APPEND);
  ...
  void ServerProcess::start() 
  {
    ...
    ACE_HANDLE fd = ACE_OS::open("server.txt", O_RDWR);
    ACE_OS::dup(fd);
    ACE_OS::dup(fd);
    ...
    sa.sa_flags = 0;
    int signals[] = { SIGQUIT, SIGILL, SIGABRT, SIGFPE, SIGSEGV,
                      SIGTERM };
	...
	while (ret == -1 && ACE_OS::last_error() == EINTR);  // ignore interrupts, e.g. when traced
	//std::cout << " ret " << ret << " " << ACE_OS::last_error() << std::endl;
	getch();
  ...
  void ServerProcess::logFileName(const char *name)
  {
    if (name && name[0]) {
      delete [] logFileName_;
      strcpy_s(logFileName_ = new char[strlen(name) + 1], strlen(name)+1, name);
    }
  }
  ...
  void ServerProcess::signalCmd(const char *cmd)
  {
    if (cmd && cmd[0]) {
      delete [] signalCmd_;
      strcpy_s(signalCmd_ = new char[strlen(cmd) + 1],strlen(cmd) + 1, cmd);
    }
  }
